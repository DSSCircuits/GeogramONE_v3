...
{
	public:
		SIMCOM(AltSoftSerial *ser);
		char atRxBuffer[INDEX_SIZE];
	//	/*uint8_t*/void init(/*unsigned long baudRate = BAUD_RATE*/);
		uint8_t checkForMessages();
		/*uint8_t*/bool deleteMessage(int);
		uint8_t readMessageBreakOut(int);
		bool prepareSMS(char * /*, uint32_t*/);
		/*uint8_t*/bool sendSMS();
		/*uint8_t*/bool deleteAllMessages();
		/*uint8_t*/bool gsmSleepMode(int);
		uint8_t signalQuality();
		/*uint8_t*/bool powerDownGSM();
		void rebootGSM();
		void initializeGSM();
		bool checkNetworkRegistration();
		uint8_t getGeo(geoSmsData *, char *);
		/*uint8_t*/bool confirmAtCommand(char *, unsigned long);
		uint8_t cipStatus();
		void getIMEI(char *);
	private:
		uint8_t powerOnGSM();
		/*uint8_t*/bool callReady();
		AltSoftSerial *GSM;
};
